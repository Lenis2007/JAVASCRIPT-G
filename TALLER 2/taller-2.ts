...
function funcion1(obligatorio: number, opcional?: boolean, porDefecto: string = "Mi nombre es Lenis"): void {
...
let flechaCon = (edad: number): string => {
...
reservada return. */
let flechaCon2 = (a: number, b: number): number => a * b;
...
let flechaMod = (a: number, b: number): number => {
    const modulo = a % b;
    return modulo;
}
